...
export async function EpicFreeGames() {
  ...
  try {
    const gameNames = await freeGamesContainer.$$eval(
      '.css-hkjq8i > div[data-testid="offer-title-info-title"]',
      (nodes) => nodes.map((node) => node.textContent)
    );
    const gameStatus = await freeGamesContainer.$$eval(
      '.css-hkjq8i > span[data-testid="offer-title-info-subtitle"]',
    ...
    const data = gameNames
      .filter((_, index) => gameStatus[index])
      .map((name, index) => ({
        name,
        status: gameStatus[index]
          ? gameStatus[index]?.split(" - ")[0]
          : "Coming Soon",
